...
- `tts.py` — 语音合成模块；调用小米 MiMo-V2.5-TTS API 将播报稿转为音频
  ...
  - 默认语音：冰糖（中文女声）
- `db.py` — SQLite 数据库模块（init_db, ensure_feed, get_latest_raw_feed, save_raw_feed, save_article, ensure_user, ensure_subscription, create_push_batch, mark_article_pushed, reset_user_history, get_user_history, get_push_batches）
...
- 七张表：
  ...
  - `push_batches` — 推送批次（user_id, created_at），每次 generate 为每个用户创建一个批次
