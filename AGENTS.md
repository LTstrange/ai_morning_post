...
# 发送已有批次的邮件（不重新生成）
uv run python main.py send 5                   # 发送批次 #5 的早报邮件（默认附带音频）
uv run python main.py send 5 --no-tts          # 不附带 TTS 音频附件
uv run python main.py send --latest            # 发送当天每个用户的最新批次
uv run python main.py send --latest --no-tts   # 当天最新批次，不带附件
